...
(function($){
	...
	$.fn.dcDrilldown = function(options) {
		...
		return this.each(function(options){
			...
			$('li .drilldownitem',$dcDrilldownObj).click(function(e){
				$link = this;
			...
			function setUpDrilldown(){
				...
				$('li',$dcDrilldownObj).each(function(){
					...
					if($('> ul',this).length){
						...
						$('> div',this).addClass(defaults.classParentLink).append($arrow);
						if(defaults.showCount == true){
							var parentLink = $('div:not(.'+defaults.classParentLink+')',this);
							var countParent = parseInt($(parentLink).length);
							getCount = countParent;
							$('> div',this).append(' <span class="'+defaults.classCount+'">('+getCount+')</span>');
						}
					}
				});
			...
			// Breadcrumbs
			//$(document).on( 'click', '#'+idHeader+' div', function(e){
			//$('#'+idHeader+' div').live('click',function(e){
			$('#dd-header-0').click(function(e) {
		...
		function checkCookie(cookieId, obj){
			...
			if(cookieVal != null){
				...
				$.each(activeArray, function(index,value){
					...
					$('> div',$cookieLi).addClass(defaults.classActive);
				});
			}
		}
		...
		function actionDrillDown(element, wrapper, obj){
			...
			if(defaults.linkType == 'link'){
				if(!$('div',$header).length){
					$($header).prepend('<ul><li><div href="#" class="first">'+defaults.resetText+'</div></li></ul>');
				}
			}
			// Update header text
		...
		function actionDrillUp(element, obj, wrapper){
			...
			if(defaults.linkType == 'breadcrumb'){
				...
				$('div:gt('+activeIndex+')',$header).remove();
			}
		}
		...
		function resetDrilldown(obj, wrapper){
			...
			$('div.'+defaults.classActive,obj).each(function(i){
